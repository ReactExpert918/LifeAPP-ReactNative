...
//		}
//
//        if (rcmessage.audioStatus == AudioStatus.AUDIOSTATUS_STOPPED) { imageViewPlay.image = RCDefaults.audioImagePlay		}
//		if (rcmessage.audioStatus == AudioStatus.AUDIOSTATUS_PLAYING) { imageViewPlay.image = RCDefaults.audioImagePause	}
//
...
class RCMessageAudioCell: RCMessageCell {
    ...
    override func bindData(_ messagesView: ChatViewController, at indexPath: IndexPath) {
        ...
        if (rcmessage.audioStatus == AudioStatus.AUDIOSTATUS_STOPPED)    { imageViewPlay.image = RCDefaults.audioImagePlay    }
        if (rcmessage.audioStatus == AudioStatus.AUDIOSTATUS_PLAYING)    { imageViewPlay.image = RCDefaults.audioImagePause    }
        if (rcmessage.audioStatus == AudioStatus.AUDIOSTATUS_PAUSED)    { imageViewPlay.image = RCDefaults.audioImagePlay     }

        viewProgress.trackTintColor = rcmessage.incoming ? RCDefaults.audioTextColorIncoming : RCDefaults.audioTextColorOutgoing
        viewProgress.progressTintColor = rcmessage.incoming ? RCDefaults.audioTextColorIncoming : RCDefaults.audioTextColorOutgoing
        ...
        activityIndicator.color = rcmessage.incoming ? RCDefaults.audioTextColorIncoming : RCDefaults.audioTextColorOutgoing
    }
    ...
    func updateDuration(_ rcmessage: RCMessage) {
        ...
        if (rcmessage.audioStatus == AudioStatus.AUDIOSTATUS_STOPPED)    { updateDuration(rcmessage.audioDuration)        }
        if (rcmessage.audioStatus == AudioStatus.AUDIOSTATUS_PLAYING)    { updateDuration(Int(rcmessage.audioDuration))    }
        if (rcmessage.audioStatus == AudioStatus.AUDIOSTATUS_PAUSED)    { updateDuration(Int(rcmessage.audioCurrent))    }
    }
