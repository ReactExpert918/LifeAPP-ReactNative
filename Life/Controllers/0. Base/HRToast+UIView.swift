...
//            color = UIColor.white // default
    ...
    fileprivate func makeToastActivity(position pos: AnyObject, message msg: String = "") {
        ...
        /*
        // MARK: - Add gif image
        let loadingGifImageView = UIImageView(frame: CGRect(x: 0, y: 0, width: HRToastActivityWidth / 2, height: HRToastActivityHeight / 2))
        loadingGifImageView.center = CGPoint(x: activityView.bounds.size.width / 2, y: activityView.bounds.size.height / 2)
        loadingGifImageView.contentMode = .scaleAspectFit
        let gifManager = SwiftyGifManager(memoryLimit:20)
        let imgLoading = UIImage(gifName: "loading.gif")
        loadingGifImageView.setGifImage(imgLoading, manager: gifManager)
        activityView.addSubview(loadingGifImageView)*/
        ...
        if (!msg.isEmpty){
            //loadingGifImageView.frame.origin.y -= 10
            //let activityMessageLabel = UILabel(frame: CGRect(x: activityView.bounds.origin.x, y: (loadingGifImageView.frame.origin.y + loadingGifImageView.frame.size.height + 10), width: activityView.bounds.size.width, height: 20))
    ...
    fileprivate func centerPointForPosition(_ position: AnyObject, toast: UIView) -> CGPoint {
        if position is String {
            ...
            if position.lowercased == HRToastPositionTop {
                return CGPoint(x: viewSize.width/2, y: 110 + HRToastVerticalMargin)
