...
class MediaUploader: NSObject {
	...
	private func upload(message: Message, completion: @escaping (_ error: Error?) -> Void) {
        ...
        if (message.type == MESSAGE_TYPE.MESSAGE_PHOTO) { uploadPhoto(message: message, completion: completion) }
		if (message.type == MESSAGE_TYPE.MESSAGE_VIDEO) { uploadVideo(message: message, completion: completion) }
