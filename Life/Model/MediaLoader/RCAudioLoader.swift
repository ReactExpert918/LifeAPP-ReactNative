...
class RCAudioLoader: NSObject {
	...
	private class func loadMedia(_ rcmessage: RCMessage, in tableView: UITableView) {
        ...
        if (network == NETWORK_MODE.NETWORK_MANUAL) || ((network == NETWORK_MODE.NETWORK_WIFI) && (Connectivity.isReachableViaWiFi() == false)) {
            rcmessage.mediaStatus = MediaStatus.MEDIASTATUS_MANUAL
		} else {
			downloadMedia(rcmessage, in: tableView)
		}
	}
	...
	private class func showMedia(_ rcmessage: RCMessage, path: String) {
        ...
        rcmessage.mediaStatus = MediaStatus.MEDIASTATUS_SUCCEED
	}
}
